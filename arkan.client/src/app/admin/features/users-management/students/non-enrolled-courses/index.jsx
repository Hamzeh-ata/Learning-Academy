...
import { selectNonEnrolledCourses, selectNonEnrolledPagination } from '@/slices/admin-slices/user-slices';
export const NonEnrolledCourses = ({ selectedCourses, setSelectedCourses, userId }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const pagination = useSelector(selectNonEnrolledPagination);
  ...
  const onPageChange = (event) => {
    ...
    dispatch(fetchNonEnrolledCourses({ UserId: userId, currentPage: event.page + 1, pageSize: event.rows }));
  };

  const filteredCourses = nonEnrolledCourses.filter((course) =>
    ...
    <div className="h-5 w-full">
      <div className="">
        <input
          ...
          className="p-inputtext p-component w-full rounded-xl"
          value={searchTerm}
